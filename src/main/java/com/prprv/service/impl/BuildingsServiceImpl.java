...
public class BuildingsServiceImpl implements BuildingsService {
    ...
    public List<Buildings> findAll() {
        List<Buildings> buildings = buildingsMapper.selectAll();
        return buildings;
    }
    ...
    public Page<Buildings> search(Map searchMap) {
        //通用Mapper多条件搜索，标准写法
        Example example = new Example(Buildings.class);//指定查询的表tb_community
        //1.初始化分页条件
        int pageNum = 1;
        int pageSize = 2;
        if(searchMap != null){
        ...
        Page<Buildings> buildings = (Page<Buildings>) buildingsMapper.selectByExample(example);
        return buildings;
    }

    @Override
    public Boolean add(Buildings buildings) {
        int row = buildingsMapper.insert(buildings);
        if(row>0){
            return true;
        }else{
            return false;
    ...
    public Buildings findById(Integer id) {
        return buildingsMapper.selectByPrimaryKey(id);
    }

    @Override
    public Boolean update(Buildings buildings) {
        int row = buildingsMapper.updateByPrimaryKeySelective(buildings);
        if(row>0){
            return true;
        }else{
            return false;
    ...
    public Boolean del(List<Integer> ids) {
        for (Integer id:ids) {
            buildingsMapper.deleteByPrimaryKey(id);
        }
        return true;
    }
}
